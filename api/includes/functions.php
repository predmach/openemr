...
function getUsernameNew($userId) {
    global $db;
...
function getPass($userId) {
      global $db;
      $strQuery = "SELECT `password` FROM users_secure WHERE id=" . $userId;
      $result = $db->get_row($strQuery);
...
function getUserData($userId) {
    $return_array = array();
    $return_array['user'] = getUsernameNew($userId);
//    $return_array['emr'] = getEmr($userId);
    $return_array['emr'] = '';
    $return_array['username'] = getUsernameNew($userId);
